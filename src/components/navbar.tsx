...
const Navbar: React.FC<NavbarProps> = ({ cartCount, onCartClick }) => {
  ...
  return (
    <>
      <nav
        className={`fixed top-0 left-0 right-0 z-50 bg-white/80 backdrop-blur-md border-b border-gray-200 transition-transform duration-300 ease-in-out ${
          ...
          <div className="flex items-center justify-between h-12">
            <div className="flex-shrink-0">
              <div className="w-5 h-6 flex items-center justify-center">
                <a href="/"><svg viewBox="0 0 24 24" className="cursor-pointer w-6 h-5 fill-current text-gray-800">
                ...
                {navItems.map((item) => (
                  <a
                    ...
                    className={`px-3 py-1 rounded-full text-sm font-medium transition-colors duration-200 ${
                      item.active ? "hover:bg-gradient-to-l bg-gradient-to-r from-blue-500 to-purple-500 text-white" : "text-gray-700 hover:text-gray-900 hover:bg-gray-100"
                    }`}
                  >
                    {item.name}
                  </a>
            ...
            <div className="md:hidden">
              <button
                onClick={toggleMenu}
                className="cursor-pointer inline-flex items-center justify-center p-2 rounded-md text-gray-700 hover:text-gray-900 hover:bg-gray-100 transition-colors duration-200"
                aria-expanded="false"
              >
      ...
      >
        ...
        <div
          className={`relative bg-white h-full w-full flex flex-col transition-transform duration-300 ease-in-out ${
            ...
            {navItems.map((item, index) => (
              <a
                ...
                className={`block text-2xl font-medium transition-colors duration-200 ${
                  item.active ? "text-blue-500" : "text-gray-900 hover:text-blue-500"
                }`}
                style={{
